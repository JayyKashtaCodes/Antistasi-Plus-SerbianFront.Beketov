...
//если есть прикрепленные объекты к технике - открепляем и удаляем их
	...
	{
		{
			...
			{
			  [typeOf _x,"Квадроцикл поставлен в гараж"] call A3A_fnc_customHint;
			  {
			    detach _y;
			    deleteVehicle _y;
			  }
...
private _addVehicle = {
	...
	//если есть прикрепленные объекты к технике - открепляем и удаляем их
	if (count attachedObjects _this > 0) then 
	{
		{
			if (typeOf _x == "UserTexture1m_F") then
			{
...
{
    if ((typeOf _x == "B_supplyCrate_F") || (typeOf _x == "Box_Syndicate_Ammo_F")) then {
		detach _x;}
	else{
		detach _x;
		_x call _addVehicle;
