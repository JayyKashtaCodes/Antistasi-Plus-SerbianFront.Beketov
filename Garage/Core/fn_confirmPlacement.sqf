...
HR_GRG_EH_keyDown = findDisplay 46 displayAddEventHandler ["KeyDown", {
    ...
    if (_key in [DIK_ESCAPE, DIK_RETURN, DIK_SPACE, DIK_Y]) then {
        ...
        private _placed = if (_key isEqualTo DIK_SPACE) then {
			...
			//create vehicle
			private _veh = createVehicle [_class, [0,0,2000], [], 0, "NONE"];
			
			//================================ TACTICAL Z ===================================================
			//			[_class, _veh] call HR_GRG_fnc_setTacticalZnak; 
			//================================ END Z ========================================================
			...
			//добавляем в меню техники действие на смену поднятого флага на флаг команды игрока
			_veh addAction ["Поднять флаг", 
			{
				[_this select 0] call nova_fnc_flagRaise;
			},nil,0,false,true,"","(isPlayer _this) and (side (group _this) == teamPlayer)"];
			
			_veh addAction ["Спустить флаг", 
			{
				[_this select 0] call nova_fnc_flagRemove;
			},nil,0,false,true,"","(isPlayer _this) and (side (group _this) == teamPlayer)"];
