...
_loadout = switch _team do {
	...
	case teamPlayer: {
		if (toLower worldName isEqualTo "enoch") then {
			...
			[[],[],[],[selectRandom (A3A_faction_reb getVariable "uniforms"), []],[],[],selectRandom (A3A_faction_civ getVariable "headgear"),"",[],
			[(selectRandom unlockedmaps),"","tf_anprc148jem",(selectRandom unlockedCompasses),(selectRandom unlockedwatches),""]];
