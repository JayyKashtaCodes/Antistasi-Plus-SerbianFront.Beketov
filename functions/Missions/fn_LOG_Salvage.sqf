...
private _text = format ["A supply shipment was sunk outside of %1. Go there and recover the supplies before %2. You will need to get a hold of a boat with a winch to recover the shipment, check beaches for civilian boats you can commandeer. <br/> <br/> Груз снабжения был потоплен за пределами <marker name='%1'>%1</marker>. Отправляйтесь туда и заберите припасы до %2. Вам нужно будет завладеть лодкой с лебедкой, чтобы забрать груз. Проверьте ближайшие пляжи на наличие гражданских лодок, которые вы можете реквизировать.", _nameDest, _displayTime];
