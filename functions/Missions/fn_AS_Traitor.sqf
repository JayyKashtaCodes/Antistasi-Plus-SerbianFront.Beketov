...
[[teamPlayer,civilian],_taskID,[format ["A traitor has scheduled a meeting with %3 in %1. Kill him before he provides enough intel to give us trouble. Do this before %2. We don't where exactly this meeting will happen. You will recognise the building by the nearby Offroad and %3 presence. <br/> <br/> Предатель запланировал встречу с %3 в <marker name='%1'>%1</marker>. Убейте его, прежде чем он передаст информацию, которая доставит нам проблемы. Сделайте это до %2. Мы не знаем, где именно произойдет эта встреча. Вы узнаете здание по рядомпроходящему бездорожью и наличию %3.",_nameDest,_displayTime,_sideName],"Kill the Traitor",_markerX],_posTsk,false,0,true,"Kill",true] call BIS_fnc_taskCreate;
[[_side],_taskID+"B",[format ["We arranged a meeting in %1 with a %3 contact who may have vital information about their Headquarters position. Protect him until %2. <br/> <br/> Мы договорились о встрече с информатором %3 в <marker name='%1'>%1</marker>. У него может быть важная информация о месторасположении штаб-квартиры противника. Защищайте его до %2.",_nameDest,_displayTime,nameTeamPlayer],"Protect Contact",_markerX],getPos _houseX,false,0,true,"Defend",true] call BIS_fnc_taskCreate;
...
if (not alive _traitor || traitorIntel) then
{
	...
	{
		{
		[petros,"hint","Someone found some intel on the traitors family, he will not cause any problems any more!"] remoteExec ["A3A_fnc_commsMP",_x],
		[petros,"hint","Кто-то нашел какую-то информацию о семье предателей, он больше не доставит проблем!"] remoteExec ["A3A_fnc_commsMP",_x],	
		}

	forEach ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits);
