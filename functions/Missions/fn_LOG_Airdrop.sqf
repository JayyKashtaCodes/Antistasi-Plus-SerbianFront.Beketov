...
private _taskText = format ["Our plane will drop some cargo at %1. Go there and throw any smoke grenade on ground, the pilot will notice the signal. Bring cargo to HQ. Do this before %2. <br/> <br/> Наш самолет готов сбросить груз в <marker name='%1'>%1</marker>. Прибудьте на место и бросьте на землю любую дымовую шашку, чтобы пилот заменит сигнал. После этого доставьте груз в штаб. Выполните это до %2.", _nameDest, _displayTime];
...
if(count _smokes > 0) then {
    ...
    sleep 2;
    private _textX = format ["This is the pilot. I see your smoke in the drop zone. I'm about to paradrop my cargo on %1 position, please wait. Пилот на связи. Вижу дым на точке %1, ждите.", mapGridPosition _positionX];
