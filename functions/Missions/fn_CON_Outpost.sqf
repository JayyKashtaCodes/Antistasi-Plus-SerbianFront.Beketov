...
if (_markerX in resourcesX) then
	{
	_textX = format ["A <marker name='%1'>%1</marker> would be a fine addition to our cause. Go there and capture it before %2. <br/> <br/> Было бы хорошо если <marker name='%1'>%1</marker> перешёл под наш контроль. Отправляйтесь туда и захватите его до %2.",_nameDest,_displayTime];
	_taskName = "Resource Acquisition  Захватите ресурсы";
	}
else
	{
	_textX = format ["A %1 is disturbing our operations in the area. Go there and capture it before %2. <br/> <br/> <marker name='%1'>%1</marker> мешает нашим делам в этом районе. Отправляйтесь туда и захватите его до %2.",_nameDest,_displayTime];
	_taskName = "Take the Outpost  Захватите блокпост";
	};
