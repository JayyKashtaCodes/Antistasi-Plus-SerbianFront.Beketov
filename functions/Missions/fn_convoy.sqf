...
switch (_convoyType) do
{
    case "Ammunition":
    {
        _textX = format ["A convoy from %1 is about to depart at %2. It will provide ammunition to %3. Try to intercept it. Steal or destroy that truck before it reaches it's destination. <br/> <br/> Колонна из <marker name='%1'>%1</marker> вот-вот отправится в <marker name='%2'>%2</marker>. Она обеспечит боеприпасами <marker name='%3'>%3</marker>. Попытайтесь перехватить её. Украдите или уничтожьте этот грузовик до того, как он доберется до места назначения.",_nameOrigin,_displayTime,_nameDest]; 
    ...
    case "Fuel":
	{
		_textX = format ["A convoy from %1 is about to depart at %2. It will provide fuel to %3. Try to intercept it. Steal or destroy that truck before it reaches it's destination. <br/> <br/> Колонна из <marker name='%1'>%1</marker> вот-вот отправится в <marker name='%2'>%2</marker>. Она обеспечит топливом <marker name='%3'>%3</marker>. Попытайтесь перехватить её. Украдите или уничтожьте этот грузовик до того, как он доберется до места назначения.",_nameOrigin,_displayTime,_nameDest];
    ...
    case "Armor":
    {
        _textX = format ["A convoy from %1 is about to depart at %2. It will reinforce %3 with armored vehicles. Try to intercept it. Steal or destroy that thing before it reaches it's destination. <br/> <br/> Колонна из <marker name='%1'>%1</marker> вот-вот отправится в <marker name='%2'>%2</marker>. Она усилит <marker name='%3'>%3</marker> бронетехникой. Попытайтесь перехватить её. Украдите или уничтожьте её до того, как она достигнет места назначения.",_nameOrigin,_displayTime,_nameDest];
    ...
    case "Prisoners":
    {
        _textX = format ["A group of POWs is being transported from %1 to %3, and it's about to depart at %2. Try to intercept it. Kill or capture the truck driver to make them join you and bring them to HQ. Alive if possible. <br/> <br/> Группу военнопленных перевозят из <marker name='%1'>%1</marker> в <marker name='%3'>%3</marker>. Колонна вот-вот отправится в <marker name='%2'>%2</marker>. Попытайтесь перехватить её. Убейте или захватите в плен водителя грузовика, чтобы заставить их присоединиться к вам и доставить их в штаб-квартиру. Живым, если это возможно.",_nameOrigin,_displayTime,_nameDest];
    ...
    case "Reinforcements":
    {
        _textX = format ["Reinforcements are being sent from %1 to %3 in a convoy, and it's about to depart at %2. Try to intercept and kill all the troops and vehicle objective. <br/> <br/> Подкрепление отправляется из <marker name='%1'>%1</marker> в <marker name='%3'>%3</marker> в составе колонны, которая вот-вот отправится в <marker name='%2'>%2</marker>. Постарайтесь перехватить и уничтожить все войска и транспортные средства цели.",_nameOrigin,_displayTime,_nameDest];
    ...
    case "Money":
    {
        _textX = format ["A truck with plenty of money is being moved from %1 to %3, and it's about to depart at %2. Steal that truck and bring it to HQ. Those funds will be very welcome. <br/> <br/> Грузовик с большим количеством денег перемещается из <marker name='%1'>%1</marker> в <marker name='%3'>%3</marker>, и он вот-вот отправится в <marker name='%2'>%2</marker>. Укради этот грузовик и доставь его в штаб-квартиру. Эти средства будут очень кстати.",_nameOrigin,_displayTime,_nameDest];
    ...
    case "Supplies":
    {
        _textX = format ["A truck with medical supplies destination %3 it's about to depart at %2 from %1. Steal that truck bring it to %3 and let people in there know it is %4 who's giving those supplies. <br/> <br/> Грузовик с медикаментами находящийся в пункте назначения <marker name='%3'>%3</marker> совсем скоро отправится в <marker name='%2'>%2</marker> из <marker name='%1'>%1</marker>. Укради этот грузовик, доставь его в <marker name='%3'>%3</marker> и дай людям там знать, что именно %4 поставляет эти припасы.",_nameOrigin,_displayTime,_nameDest,nameTeamPlayer];
...
private _fnc_spawnConvoyVehicle = {
    ...
    _pathState = [_route, [20, 60] select (count _pathState == 0), _pathState] call A3A_fnc_findPosOnRoute;
    while {true} do {
...
private _fnc_applyResults =
{
    ...
    [2, format ["Rebels %1 a %2 convoy mission", ["lost", "won"] select _success, _type], _filename, true] call A3A_fnc_log;
};
...
if (_convoyType == "Fuel") then
{
	...
	if ((_vehObj distance _posDest < _arrivalDist) or (time > _timeout)) then
    {
        [true, false, 0, 5*_bonus, 10, 90, "fuel"] call _fnc_applyResults;
        [0,10*_bonus,_posSpawn] remoteExec ["A3A_fnc_citySupportChange",2];
        {if (_x distance _vehObj < 500) then {[10*_bonus,_x] call A3A_fnc_playerScoreAdd}} forEach (allPlayers - (entities "HeadlessClient_F"));
    }
    else
