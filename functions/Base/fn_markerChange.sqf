...
//проверяем в радиусе 300 метров врагов, приказываем им сдаться
if ((_winner == Invaders) or (_winner == teamPlayer)) then 
{
	[2, format [localize "STR_RUSSIAN_TAKEOVER", _markerX], _fileName] call A3A_fnc_log;
			{
				if (_x distance getMarkerPos _markerX < 200) then {
					_x setVariable ["surrendering",true,true];
					[_x] spawn A3A_fnc_surrenderAction;
					[2, format [localize "STR_RUSSIAN_SUS", _x], _fileName] call A3A_fnc_log;
					sleep 2;
				}
			} forEach (allUnits select {(side group _x == Occupants) and ([_x] call A3A_fnc_canFight)});
};
//----------------------------------------------------------------------
...
if (_winner == teamPlayer) then
{
	...
	if (!isNull _flagX) then
	{
		...
		//[_flagX,SDKFlagTexture] remoteExec ["forceFlagTexture",_flagX];
		//_flagX forceFlagTexture SDKFlagTexture;
	...
	if (_looser == teamPlayer) then {
        ...
        ([Invaders] + _prestigeInvaders) spawn A3A_fnc_addAggression;
		if (gamemode == 5) then {
		  if ((random 10 < ((tierWar + difficultyCoef)/4)) and !("DEF_HQ" in A3A_activeTasks) and (isPlayer theBoss)) then {[[Occupants],"A3A_fnc_attackHQ"] remoteExec ["A3A_fnc_scheduler",2]};
		}
		else {
