...
[_sideTsk,_taskId,[format ["%2 атакуют из %1. Перехватите их или мы можем потерять сектор",_nameOrigin,_nameENY],format ["%1 Attack",_nameENY],_mrkOrigin],getMarkerPos _mrkOrigin,false,0,true,"Defend",true] call BIS_fnc_taskCreate;
[_sideTsk1,_taskId+"B",[format ["Русские атакуют %2 из %1. Помогите операции, если можете",_nameOrigin,_nameDest],format ["%1 Attack",_nameENY],_mrkDestination],getMarkerPos _mrkDestination,false,0,true,"Attack",true] call BIS_fnc_taskCreate;
...
while {(_waves > 0)} do
{
	...
	private _pathState = [];			
	private _fnc_spawnConvoyVehicle = // Функция выстраивания колонны наземной техники на дороге
	{	
	...
	if !(_mrkDestination in blackListDest) then {
			...
			and ([_posOrigin, _posDestination] call A3A_fnc_arePositionsConnected)) then
		{
			...
			[2, format ["Выбран аэропорт для наземной атаки %1", _mrkOrigin], _filename] call A3A_fnc_log;
		}
		else
		//Find an outpost we can attack from
		{
			...
			[2, format ["Аутпосты для наземной атаки %1", _outposts], _filename] call A3A_fnc_log;
			if !(_outposts isEqualTo []) then
			{
				...
				_pos = position _spawnPoint;
				_dir = getDir _spawnPoint;
				[2, format ["Выбран аутпост для наземной атаки %1", _outpost], _filename] call A3A_fnc_log;
			};
		};
	};
	private _nVehLand = 0;	//отправка наземной техники
	if !(_posOriginLand isEqualTo []) then
	{
		...
		while {_countX <= _nVehLand} do
		{
			if (count _vehPoolLand == 0) then {
				...
				_waves = 0;
				[2, "В атаке закончилась наземная техника", _filename] call A3A_fnc_log;
			};
	...
	if !(canMove _uav) then //отправка беспилотника с шансом 75%
	{
	...
	while {_countX <= _nVehAir} do	//отправка самолетов и вертолетов
	{
		private _typeVehX = "";
		...
		if (true) then
			...
			{[_x] call A3A_fnc_NATOinit} forEach units _groupVeh;
			[_veh, _sideX] call A3A_fnc_AIVEHinit;
			if (not (_typeVehX in vehTransportAir)) then //это отправляет боевую авиацию
				{
				...
				*/
			...
			else //здесь отправляет транспортное воздушное судно (может быть и вертолет и самолет транспортный)
				{		//создаем группу бойцов
				_groups pushBack _groupVeh;
				_typeGroup = [_typeVehX,_sideX] call A3A_fnc_cargoSeats;
				_grupo = grpNull;
				...
				{	//сажаем бойцов в транспортное воздушное судно
					_x assignAsCargo _veh;
					_x moveInCargo _veh;
					if (vehicle _x == _veh) then
						{
				...
				if (!(_veh isKindOf "Helicopter") or (_mrkDestination in airportsX)) then		//если транспортное воздушное судно не вертолет (а танспортный самолет) или точка назначения аэропорт
					{
					[_veh,_grupo,_mrkDestination,_mrkOrigin] spawn A3A_fnc_paradrop;		//запускаем фуккцию парашютирования над точкой назначения
					}
				else
					{
					
					_landPos = _posDestination getPos [150, random 360]; 
					_landPos = [_landPos, 0, 300, 20, 0, 0.2, 0, [], [[0,0,0],[0,0,0]]] call BIS_fnc_findSafePos; //найти позицию в радиусе 550 метров в отдалении 150 метров от точки назначения
					if !(_landPos isEqualTo [0,0,0]) then //если позиция для высадки найдена отправляем вертолет на высадку десанта
						{
						_landPos set [2, 0];
						_pad = createVehicle ["Land_HelipadEmpty_F", _landPos, [], 0, "NONE"];
						
						[2, format ["Запускаем транспортный вертолет в позицию %1", _landPos], _filename] call A3A_fnc_log;
						
						_vehiclesX pushBack _pad;
					
											
						//путевые точки для пилотов
						private _mywp0 = _groupVeh addWaypoint [_landpos, 0];
						_mywp0 setWaypointType "TR UNLOAD"; //прилететь выгрузить десант
						...
						_mywp2 setWaypointType "MOVE";	//улететь обратно на базу
						...
						*/
						...
						//Set the waypoints for cargoGroup  / Путевые точки для десанта
						private _cargoWP0 = _grupo addWaypoint [_landpos, 0];
						_cargoWP0 setWaypointType "GETOUT";	//высадиться с вертолета
						...
						_cargoWP1 setWaypointType "SAD";	//найти и уничтожить врага на точке
						_cargoWP1 setWaypointBehaviour "AWARE";
						_cargoWP1 setWaypointSpeed "FULL";

						
						}
					else //если места для высадки нет
						{
						...
						if ((_typeVehX in vehFastRope) and ((count(garrison getVariable [_mrkDestination, []])) < 10)) then
							{
							//_grupo setVariable ["mrkAttack",_mrkDestination];
							[_veh,_grupo,_posDestination,_posOrigin,_groupVeh] spawn A3A_fnc_fastrope;		//спускаем десант на веревках если транспортное воздушное судно поддерживает "быстрые веревки"
							}
						else
							{
							[_veh,_grupo,_mrkDestination,_mrkOrigin] spawn A3A_fnc_paradrop;		//иначе сбрасываем дасант на парашютах
							}
						};
					};
				};
...
[3600, _originalSide] remoteExec ["A3A_fnc_timingCA", 2]; //добавляет 60 минут перед следующей атакой
bigAttackInProgress = false; publicVariable "bigAttackInProgress";  //выключаем выключатель Большой атаки
forcedSpawn = forcedSpawn - [_mrkDestination]; publicVariable "forcedSpawn";


// Hand remaining aggressor units to the group despawner - Запускаем процедуру удаления групп атакующих
{
