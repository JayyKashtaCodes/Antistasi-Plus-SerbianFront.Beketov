...
[[teamPlayer,civilian,Occupants],_taskId,[format ["%2 is attacking critical positions within %1! Defend the city at all costs <br/> <br/> %2 атакует важные позиции в пределах <marker name='%1'>%1</marker>! Защитите город любой ценой",_nameDestination,nameInvaders],format ["%1 Punishment",nameInvaders],_attackDestination],getMarkerPos _attackDestination,false,0,true,"Defend",true] call BIS_fnc_taskCreate;
...
if (({_x call A3A_fnc_canFight} count _soldiers < count _soldiers / 3) or (time > _missionExpireTime)) then {
    [
        2,
        format ["Rebels defeated a punishment attack against %1", _attackDestination],
        _fileName
    ] call A3A_fnc_log;
    ...
    [
        2,
        format ["Rebels lost a punishment attack against %1", _attackDestination],
        _fileName
    ] call A3A_fnc_log;
