...
} forEach (allMissionObjects "UserTexture1m_F");	// очищаем все тактические знаки в миссии не прикрепленные к объектам (при активации аэропорта бывает остаются тактические знаки с уничтоженной техники после нападения на аэропорт)
...
while {_spawnParameter isEqualType []} do {
    {
        if(_x != "") then {
            private _vehiclePosition = [_spawnParameter select 0, 25, 150, 20, 0, 0.7] call BIS_fnc_findSafePos;
...
if (!isNil "_selectedVehicle") then {
	private _patrolPos = [_positionX, 30, _size, 15, 0, 0.5, 0, [], [_positionX, _positionX]] call BIS_Fnc_findSafePos;
	...
	while {_countX < 4} do
	{
		...
		if(_spawnParameter isEqualType []) then
		{
			...
			if(count _vehPool > 0) then
			{
				...
				_veh = createVehicle [_typeVehX, (_spawnParameter select 0), [], 0, "CAN_COLLIDE"];//"CAN_COLLIDE"
...
if (!_busy) then
{
	for "_i" from 1 to (round (random 2)) do
	{
		...
		{
			_pos = (_spawnParameter select 0);
			_veh = createVehicle [_typeVehX, [0,0,2000] , [], 0, "NONE"]; //"CAN_COLLIDE"
			
			//===================================================== TACTICAL Z ============================================================			
			//if (_sideX != Occupants) then {[_typeVehX, _veh] call HR_GRG_fnc_setTacticalZnak; sleep 3;};
			//===================================================== END Z =================================================================
...
while {_countX < _nVeh && {_countX < 3}} do
{
	...
	{
		_pos = (_spawnParameter select 0);
		_veh = createVehicle [_typeVehX, [0,0,2000], [], 0, "NONE"];
...
} forEach (allMissionObjects "UserTexture1m_F");				// очищаем все тактические знаки в миссии не прикрепленные к объектам
