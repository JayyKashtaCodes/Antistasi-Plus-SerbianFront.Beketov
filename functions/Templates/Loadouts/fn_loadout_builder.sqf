...
//Adds a Googles to the loadout, selected at random from the category in loadout data.
private _fnc_setGoogles = {
	params ["_key"];
	private _data = _loadoutDataForTemplate getVariable [_key, []];
	if (_data isEqualTo []) exitWith {};
	private _googles = selectRandom _data;
	[_finalLoadout, _googles] call A3A_fnc_loadout_setGoogle;
};
...
//Adds a voice to the unit, select at random from the given category of loadout data.
private _fnc_addVoice = {
	params ["_key"];
	private _data = _loadoutDataForTemplate getVariable [_key, []];
	if (_data isEqualTo []) exitWith {};
	private _voice = selectRandom _data;
	_equipment pushBack ["VOICE", _voice];
};
