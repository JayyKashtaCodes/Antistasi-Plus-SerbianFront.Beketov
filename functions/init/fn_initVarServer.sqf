...
private _magazineVars = ["SDKMortarHEMag", "SDKMortarSmokeMag", "vehNATOMRLSMags", "vehCSATMRLSMags", "breachingExplosivesAPC", "breachingExplosivesTank", "NATOmortarMagazineHE", "NATOHowitzerMagazineHE", "CSATmortarMagazineHE", "CSATHowitzerMagazineHE","ATMineMags","APERSMineMags"];
...
if (!(shop_tank isEqualTo [])) then {
	...
	private _fourthTank = shop_tank select 3;
	if (!isNil "_fourthTank") then {
		server setVariable [_fourthTank, 33000, true];
	};
};
...
if (!(additionalShopArtillery isEqualTo [])) then {
	...
	if (A3A_hasRHS) then {
		server setVariable [(additionalShopArtillery select 0), 25000, true];
	};
};
